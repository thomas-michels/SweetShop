...
class TestProductAdditionalRepository(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_create_product_additional(self):
        ...
        result = await self.repo.create(group, product_id="prod_1")
    ...
    async def test_update_product_additional(self):
        created = await self.repo.create(await self._group(name="Old"), product_id="prod_1")
        created.name = "New"
    ...
    async def test_delete_by_id(self):
        created = await self.repo.create(await self._group(name="Del"), product_id="prod_1")
    ...
    async def test_select_all(self):
        await self.repo.create(await self._group(name="G1"), product_id="prod_1")
        await self.repo.create(await self._group(name="G2"), product_id="prod_1")
        results = await self.repo.select_all()
        self.assertEqual(len(results), 2)

    async def test_select_by_ids(self):
        g1 = await self.repo.create(await self._group(name="G1"), product_id="prod_1")
        g2 = await self.repo.create(await self._group(name="G2"), product_id="prod_1")
