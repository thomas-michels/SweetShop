...
class ExpenseServices:
    ...
    async def create(self, expense: Expense) -> ExpenseInDB:
        ...
        total_paid = self.__calculate_total_paid(
            payment_details=expense.payment_details
        )

        if total_paid <= 0:
            raise UnprocessableEntityException(
                detail="Total paid should be grater than zero"
            )

        expense_in_db = await self.__expense_repository.create(
            expense=expense, total_paid=total_paid
        )
    ...
    async def update(self, id: str, updated_expense: UpdateExpense) -> ExpenseInDB:
        ...
        is_updated = expense_in_db.validate_updated_fields(
            update_expense=updated_expense
        )

        if updated_expense.payment_details is not None:
            total_paid = self.__calculate_total_paid(
                payment_details=updated_expense.payment_details
            )
        ...
        if is_updated:
            ...
            expense_in_db = await self.__expense_repository.update(
                expense=expense_in_db
            )
