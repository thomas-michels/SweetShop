"""Facade responsible for orchestrating order operations."""
...
from ..domain.factory import OrderFactory
from ..domain.order import DeliveryData, Order, OrderData
from ..schemas import (
    CompleteOrder,
    DeliveryType,
...
class OrderFacade:
    """High level API for order operations used by the application layer."""
    ...
    def __init__(
        ...
        self.__cache_tags: Dict[str, Any] = {}
        self.__cache_customers: Dict[str, Any] = {}
    ...
    async def create(self, order: RequestOrder) -> CompleteOrder:
        ...
        delivery_value = (
            order.delivery.delivery_value if order.delivery.delivery_value is not None else 0
        )
    ...
    async def update(self, id: str, updated_order: UpdateOrder) -> CompleteOrder:
        ...
        if updated_order.tags is not None:
            temp_tags = list(updated_order.tags)
        ...
        order_domain = self.__build_domain_order(order_in_db)

        if updated_order.status is not None:
            await order_domain.change_status(updated_order.status)
            updated_fields["status"] = order_domain.status.current_status.value
        ...
        if is_updated or updated_fields.get("products"):
            ...
            payload = updated_order.model_dump(exclude_none=True)
            payload.pop("status", None)
            updated_fields.update(payload)
    ...
    async def search_by_id(self, id: str, expand: List[str] | None = None) -> CompleteOrder:
        expand = expand or []
    ...
    async def search_all(
        ...
        await self.__preload_customers_if_needed(orders, expand)
    ...
    async def __build_complete_order(
        self, order_in_db: OrderInDB, expand: List[str] | None = None
    ) -> CompleteOrder:
        expand = expand or []
        ...
        if "customers" in expand and order_in_db.customer_id is not None:
            customer = await self.__get_customer(order_in_db.customer_id)
            if customer:
                complete_order.customer = customer

        if "tags" in expand:
            complete_order.tags = await self.__get_tags(order_in_db.tags)

        return complete_order

    async def __validate_products(
        self, raw_products: List[RequestedProduct]
    ) -> List[StoredProduct]:
    ...
    async def __get_customer(self, customer_id: str):
        if customer_id not in self.__cache_customers:
            customer = await self.__customer_repository.select_by_id(
                id=customer_id, raise_404=False
            )
            self.__cache_customers[customer_id] = customer
        return self.__cache_customers.get(customer_id)

    async def __get_tags(self, tags: List[str]):
        result = []
        for tag in tags:
            ...
            tag_in_db = self.__cache_tags.get(tag)
            if tag_in_db:
                result.append(tag_in_db)
        return result

    async def __preload_customers_if_needed(
        self, orders: List[OrderInDB], expand: List[str]
    ) -> None:
        if "customers" not in expand:
            return
        ...
        customers = await self.__customer_repository.select_by_ids(list(customer_ids))

        for customer in customers:
            self.__cache_customers[customer.id] = customer

    def __build_domain_order(self, order_in_db: OrderInDB) -> Order:
        delivery = order_in_db.delivery
        delivery_data = DeliveryData(
            delivery_type=delivery.delivery_type,
            delivery_value=delivery.delivery_value,
            delivery_at=delivery.delivery_at,
            address=self.__serialize_address(delivery.address),
        )
                    ...
                    "additionals": [
                        {
                            ...
                            "unit_cost": additional.unit_cost,
                            "consumption_factor": additional.consumption_factor,
                            "quantity": additional.quantity,
                        }
                        for additional in product.additionals
                    ],
        ...
        payments = [payment.model_dump() for payment in order_in_db.payments]
        ...
        return OrderFactory.build(order_data, observers=[self.__order_message_manager])

    @staticmethod
    def __serialize_address(address: Any) -> Any:
        if address is None:
            return None
        if hasattr(address, "model_dump"):
            return address.model_dump()
        if hasattr(address, "dict"):
            return address.dict()
        return address
