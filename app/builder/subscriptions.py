...
class SubscriptionBuilder:
    ...
    async def subscribe(self, subscription: RequestSubscription, user: UserInDB) -> ResponseSubscription:
        ...
        if active_plan:
            raise BadRequestException("Organização já possui um plano ativo. Use 'alterar plano' para atualizar.")

        # Cria um novo plano começando hoje
        today = datetime.now().date()
        start_date = datetime.combine(today, datetime.min.time())  # 00:00:00 de hoje
        ...
        if subscription.cupoun_id:
            ...
            coupon_in_db = await self.__coupon_service.update_usage(coupon_id=coupon_in_db.id, quantity=1)
            ...
            _logger.info(f"Desconto de R${discount} aplicado usando cupom {subscription.cupoun_id}")
